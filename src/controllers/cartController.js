...
const createCart = async (req, res) => {
  try {
    ...
    if (!validator.isValidBody(data)) {
      return res
        .status(400)
        .send({ status: false, msg: "please provide Cart details" });
    }
    ...
    if (isNaN(Number(quantity))) {
      return res
        .status(400)
        .send({ status: false, message: "quantity should be a valid number" });
    }

    if (quantity < 0) {
      return res
        .status(400)
        .send({ status: false, message: "quantity can not be less than zero" });
    }
    ...
    if (isOldUser) {
      ...
      for (let i = 0; i < items.length; i++) {
        if (items[i].productId.toString() === productId) {
          console.log(chalk.bgBlue("productId are similar"));
        ...
        const saveData = await cartModel
          .findOneAndUpdate(
            { userId: userIdFromParams },
            {
              ...
              $inc: { totalItems: 1, totalPrice: findProduct.price * quantity },
            },
            { new: true }
          )
          .select({ "items._id": 0 });
...
const updateCart = async (req, res) => {
  try {
    ...
    if (Object.keys(data).length <= 0) {
      return res
        .status(400)
        .send({ status: false, msg: "Plz enter data in body !!!" });
    }
    if (!validator.isValid(userIdFromParams)) {
      return res
        .status(400)
    ...
    if (!findProductInCart) {
      return res
        .status(404)
        .send({ status: false, message: "product not found in the cart." });
    }

    if (!validator.isValid(removeProduct)) {
      return res
        .status(400)
        .send({ status: false, message: "please provide items to delete" });
    }
    ...
    if (removeProduct == 1) {
      console.log(chalk.bgBlackBright("quantity decrement starts now"));
...
const getCart = async (req, res) => {
  try {
    ...
    if (!checkCart) {
      return res
        .status(404)
        .send({ status: false, message: "Cart Not Exist With This User" });
    }
    return res
      .status(200)
      .send({ status: false, message: "User Cart Details", data: checkCart });
  } catch (error) {
...
const delCart = async (req, res) => {
  try {
    ...
    if (!checkUser) {
      return res
        .status(404)
        .send({ status: false, message: "This User is Not Exist" });
    }
    ...
    if (!checkCart) {
      return res
        .status(404)
        .send({ status: false, message: "Cart does Not Exist With This User" });
    }
