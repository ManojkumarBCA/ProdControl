...
const postProducts = async function (req, res) {
  try {
    ...
    if (!validUrl.isUri(uploadedFileURL)) {
      return res
        .status(400)
        .send({ status: false, msg: "invalid uploadFileUrl" });
    }
...
const getIdproducts = async (req, res) => {
  try {
    ...
    //check whether objectId is valid or not--
    ...
    if (!product) {
      return res
        .status(404)
        .send({ status: false, message: "No Products Available!!" });
    }
  ...
  try {
    ...
    if (findTitle) {
      return res
        ...
        .send({
          status: false,
          msg: "Title Is Already Exists, Please try different One!!!",
        });
    }
    ...
    if (price) {
      if (!validator.isValidPrice(price)) {
        return res
          ...
          .send({
            status: false,
            msg: "price should be in valid format like numeric or upto two decimal places format!!!",
          });
      }
    }
      ...
      for (let i = 0; i < values.length; i++) {
        if (
          ...
          return res
            ...
            .send({
              status: false,
              msg: "Plz Enter availableSizes From S, XS, M, X, L, XXL, XL",
            });
        }
      }
    }
    ...
    if (installments) {
      if (isNaN(installments) == true) {
        return res
          ...
          .send({
            status: false,
            msg: "Please Enter Number In installments !!!",
          });
      }
    }
    ...
    if (!searchProduct) {
      return res
        .status(404)
        .send({ status: false, msg: "ProductId does not exist" });
    }
...
const deleteById = async function (req, res) {
  try {
    ...
    if (!deletedProduct) {
      return res
        .status(404)
        .send({ status: false, msg: `this ${productId} is not exist in db` });
    }

    if (deletedProduct.isDeleted !== false) {
      return res
        .status(404)
        .send({ status: false, msg: `this ${productId} is Not Found` });
    }
