  ...
  try {
    ...
    if (userId.length < 24 || userId.length > 24) {
      return res.status(400).send({
        status: false,
        msg: "Plz Enter Valid Length Of userId in Params !!!",
      });
    }
    ...
    if (!cartId) {
      return res
        .status(400)
        .send({ status: false, msg: "Plz enter cartId in body !!!" });
    }
    if (cartId.length < 24 || cartId.length > 24) {
      return res.status(400).send({
        status: false,
        msg: "Plz Enter Valid Length Of cartId in Body !!!",
      });
    }
    ...
    if (!userCart) {
      return res
        .status(404)
        .send({ status: false, msg: "User does not have any cart !!!" });
    }
...
const putOrder = async function (req, res) {
  try {
    ...
    if (!orderId) {
      return res
        .status(400)
        .send({ status: false, msg: "Plz enter orderId in body !!!" });
    }
    if (!status) {
      return res
        .status(400)
        .send({ status: false, msg: "Plz enter status in body !!1" });
    }
    ...
    if (!orderFind) {
      return res
        .status(404)
        .send({ status: false, msg: "Order not found !!!" });
    }
    ...
    if (orderFind.cancellable == true) {
      if (orderFind.status == "pending") {
        ...
        if (!updateStatus) {
          return res.status(400).send({
            status: false,
            message: "Won't able to change status !!!",
          });
        }
      ...
      if (orderFind.status == "completed") {
        return res.status(400).send({
          status: false,
          message: "Order already completed, won't able to change status !!!",
        });
      }
      ...
      if (orderFind.status == "canceled") {
        return res
          .status(400)
          .send({ status: false, message: "Order already canceled !!!" });
      }
    }
    ...
    if (orderFind.cancellable == false) {
      return res
        .status(400)
        .send({ status: false, msg: "its not cancellable !!!" });
    }
  } catch (err) {
    res.status(500).send({ status: false, msg: err.message });
  }
